...
module.exports = {
  async up(queryInterface) {
    ...
    await q(`
      CREATE PROCEDURE sp_get_siniestros_ajustador(IN p_ajustador_id INT)
      BEGIN
        SELECT
          s.id                                          AS id,
        ...
        WHERE s.ajustador_id = p_ajustador_id
        ORDER BY s.id DESC;
      END
    `);
  },
  ...
  async down(queryInterface) {
    ...
    await q(`
      ...
      BEGIN
        SELECT
          s.id, s.numero_reporte, s.fecha_hora_siniestro, s.latitud, s.longitud,
          s.conductor_momento, s.descripcion_hechos, s.dictamen_supervisor,
          s.presupuesto_reparacion, s.perdida_total,
          ces.descripcion_interna AS estatus, ces.color_ui AS estatus_color,
          CONCAT(duenio.nombre, ' ', duenio.apellidos) AS duenio_nombre,
          duenio.email AS duenio_email,
          v.marca, v.modelo, v.anio, p.placas, p.numero_poliza,
          p.fecha_inicio, p.fecha_fin, cs.nombre_comercial AS compania,
          CONCAT(aj.nombre, ' ', aj.apellidos) AS ajustador_nombre
        FROM siniestros s
        INNER JOIN polizas p                        ON p.id = s.poliza_id
        INNER JOIN usuarios duenio                  ON duenio.id = p.usuario_id
