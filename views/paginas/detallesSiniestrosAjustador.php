...
<main class="min-h-[calc(100vh-180px)] bg-[#e6e7e2] px-6 py-12 font-sans text-[#111823]">
    <div class="mx-auto mb-6 flex max-w-[1200px] items-center justify-between">
        <a href="/siniestrosAjustadores" class="flex items-center gap-2 text-[13px] font-bold text-[#111823] no-underline hover:opacity-70">
            ← Volver
        </a>
    ...
    <div class="mx-auto flex max-w-[1200px] flex-col gap-10 lg:flex-row lg:gap-16">
        ...
        <div class="relative flex w-full flex-col items-center space-y-6 lg:w-24">
            ...
            <div class="mt-8 pt-4">
                <a href="/chat?siniestro_id=<?= (int)$siniestro['id'] ?>"
                        ...
                        <div class="flex flex-col gap-2">
                            <?php foreach ($terceros as $t): ?>
                                <div class="rounded-[16px] bg-[#b8bec8] px-5 py-3 text-[13px]">
                                    <p class="font-bold text-[#111823]">
                                        <?= htmlspecialchars($t['marca_tercero']) ?>
                                        <?= htmlspecialchars($t['modelo_tercero']) ?>
                                        — <?= htmlspecialchars($t['placas_tercero']) ?>
                                    </p>
                                    <?php if (!empty($t['aseguradora_tercero'])): ?>
                                        <p class="text-[#4a5568]">Aseguradora: <?= htmlspecialchars($t['aseguradora_tercero']) ?></p>
                                    <?php endif; ?>
                                    <?php if (!empty($t['descripcion_danos'])): ?>
                                        <p class="text-[#4a5568]">Daños: <?= htmlspecialchars($t['descripcion_danos']) ?></p>
                                    <?php endif; ?>
                                </div>
                            <?php endforeach; ?>
                        </div>
